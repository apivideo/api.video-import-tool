...
import {
  ...
  getOauthAccessTokenCall,
  RevokeAccessTokenResponse,
  revokeOauthAccessTokenCall,
...
class ZoomProviderService implements AbstractProviderService {
  authenticationContext?: ClearProviderAuthenticationContext;
  ...
  constructor(
    ...
    this.authenticationContext = clearAuthenticationContext;
  }
  ...
  public async revokeOauthAccessToken(): Promise<RevokeAccessTokenResponse> {
    return await revokeOauthAccessTokenCall(
      "https://zoom.us/oauth/revoke",
      ZOOM_CLIENT_ID,
      ZOOM_CLIENT_SECRET,
      this.authenticationContext?.publicData!
    );
  }
  ...
  public async getImportableVideos(
    ...
    const zoomRes: ZoomRecordingsApiResponse = await this.callApi(
      `https://api.zoom.us/v2/accounts/me/recordings?${queryParams}`,
      "GET"
    );
        ...
        .map((meeting) =>
          meeting.recording_files
            ...
            .map((recording) => ({
              ...
              name: meeting.topic,
              thumbnail: "/zoom.svg",
              url: `${recording.download_url}?access_token=${this.authenticationContext?.clearPrivateData}`,
  ...
  private async callApi(
    ...
    headers.append(
      "Authorization",
      "Bearer " + this.authenticationContext.clearPrivateData
    );
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Zoom API call failed: ${res.statusText}. Response: ${responseText}`
      );
    }
