...
class GcsProviderService implements AbstractProviderService {
  ...
  constructor(
    ...
    const parsed = JSON.parse(clearAuthenticationContext?.clearPrivateData!)
  ...
  public async getPublicMp4Url(videoSource: VideoSource): Promise<VideoSource> {
    const apiRes = await this.callApi(
      `https://api.mux.com/video/v1/assets/${videoSource.id}`,
      "GET"
    );
  ...
  public async generatePublicMp4(
    ...
    if (!videoSource.url || videoSource.url.length === 0) {
      await this.callApi(
        `https://api.mux.com/video/v1/assets/${videoSource.id}/master-access`,
        "PUT",
        {
  ...
  public async validateCredentials(): Promise<CredentialsValidationResult> {
    ...
    return {
      encryptedPrivateData: encrypt(
        JSON.stringify({ accessKey: this.accessKey, secretKey: this.secretKey })
      ),
    };
  }
  ...
  public async getImportableVideos(
    ...
    const apiRes = await this.callApi(
      "https://api.mux.com/video/v1/assets" +
        (nextPageFetchDetails?.page
          ? `?page=${nextPageFetchDetails?.page}`
          : ""),
      "GET"
    );
