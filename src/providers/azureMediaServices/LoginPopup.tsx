...
const AzureLoginPopup = (props: S3LoginPopupProps) => {
  ...
  const onSubmit = async () => {
    if (!sasUrl || !envFile) {
      setError("SAS URL and Credentials JSON file are required");
      return;
    }
    try {
      const parsedJson = JSON.parse(envFile);
    } catch (e) {
    ...
    const res = await callValidateProviderCredentialsApi({
      provider: "AZURE_MEDIA",
      clearAuthenticationContext: {
        clearPrivateData: JSON.stringify({
          sasUrl,
          credentials: JSON.parse(envFile),
        }),
      },
    });
    if (res.error) {
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  id="jsonCredentials"
                  placeholder=""
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-s3 focus:border-s3hover block w-full p-2.5"
                  required
                />
              </div>
              <div>
              ...
              <p className="mt-2">
                <i>
                  ...
                  <a
                    target="_blank"
                    href="/doc/azure-media-services-credentials"
